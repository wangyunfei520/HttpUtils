...
//    private static final String[] DUNIT = {"角", "分"};
    ...
    public static String toChinese(String str) {
        ...
        if (chineseInteger == null || chineseInteger.length() == 0){
            chineseInteger = NUMBERS[0] + "点";
            if (chineseDecimal == null || chineseDecimal.length() == 0){
                return NUMBERS[0] + IUNIT[0];
            }
        }
        if (chineseDecimal != null && chineseDecimal.length() > 0
                && !chineseDecimal.equals(NUMBERS[0] + NUMBERS[0])
                && !chineseDecimal.equals(NUMBERS[0])){
            chineseInteger = chineseInteger.replace(IUNIT[0],"点");//将元变为点
            return chineseInteger + chineseDecimal + IUNIT[0];
        }
...
//                    : (NUMBERS[decimals[i]] + DUNIT[i]));
            chineseDecimal.append(NUMBERS[decimals[i]]);
        }
    ...
    public static String toSimplifyChinese(String amount) {
        return toSimplified(toChinese(amount));
    }
